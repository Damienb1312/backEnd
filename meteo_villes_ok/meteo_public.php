...
if (!empty($_POST['insee'])) {
    ...
    if ($data) {
        ...
        $meteoDisplayed = "Aujourd'hui à <strong>{$city->name}</strong>, on prévoit <strong>{$forecast->rr10}mm</strong> 
        (pas plus de {$forecast->rr1}mm en tous cas) de précipitations. Météo du jour : {$forecast->weather}";
    }
}
?>
